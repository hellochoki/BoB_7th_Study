...
void bars(float percentage, float maxPercentage, int maxWordLen, bool isScaled){
	...
	for (bar = 0; bar < (int)((percentage/maxPercentage)*maxBarWidth); bar++){	
		printf("\u2591");
	}
}
...
void rowBar(word *current,int maxWordLen,bool isScaled){
	...
	word *temp = wordsList;
	while (temp != NULL) { 
        totalcnt += temp->frequency; 
        temp = temp->next; 
    } 

	percentage = (current->frequency*100.0)/totalcnt;

	maxPercentage = (wordsList->frequency*100.0)/totalcnt;	//taking max percentage
